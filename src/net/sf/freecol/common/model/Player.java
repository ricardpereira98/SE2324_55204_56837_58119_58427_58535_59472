...
public class Player extends FreeColGameObject implements Nameable {
    ...
    private final UnitIterator nextActiveUnitIterator = new UnitIterator(this, Unit::isCandidateForNextActiveUnit);
    ...
    public void changeNationType(NationType newNationType) {
        if (this.nationType != null)
            removeFeatures(this.nationType);
    ...
    public void setNation(Nation newNation) {
        ...
        java.util.Map<Nation, NationOptions.NationState> nations = getGame().getNationOptions().getNations();
    ...
    public List<AbstractUnit> getMilitaryUnits() {
        ...
        java.util.Map<UnitType, HashMap<String, Integer>> unitHash = new HashMap<>(milUnits.size());
        ...
        forEachMapEntry(unitHash, te -> forEachMapEntry(te.getValue(),
                re -> units.add(new AbstractUnit(te.getKey(), re.getKey(), re.getValue()))));
        return units;
    }
    ...
    public Goods getMostValuableGoods() {
        ...
        if (market == null)
            return null;
        final Predicate<Goods> boycottPred = g -> getArrears(g.getType()) <= 0 && hasTraded(g.getType());
        final Comparator<Goods> tradedValueComp = Comparator.comparingInt(g -> market.getSalePrice(g.getType(),
        ...
        synchronized (this.settlements) {
            if (this.colonyComparator == null) {
                ...
                while (i >= 0) {
                    ...
                    if (this.colonyComparator.compare(c, s) >= 0)
                        break;
                    i--;
                }
    ...
    public List<IndianSettlement> getIndianSettlementsWithMissionaryList(Player p) {
        final Predicate<Settlement> isPred = s -> s instanceof IndianSettlement
    ...
    public Set<Tile> getVisibleTileSet() {
        ...
        if (spec.getBoolean(GameOptions.FOG_OF_WAR)) {
            ...
            List<? extends Settlement> settlements = (hasAbility(Ability.SEE_ALL_COLONIES))
     ...
     *         int potential = market.getSalePrice(type,
     *         tile.getPotentialProduction(type, null));
    ...
    public List<Double> getAllColonyValues(Tile tile) {
        ...
        final Predicate<Unit> hostileUnitPred = u -> !this.owns(u) && u.isOffensiveUnit()
